...
def deletar_produto(id_produto):
    """DELETE: Remove um produto do banco de dados definitivamente."""
    conexao = sqlite3.connect('sistema_estoque.db')
    cursor = conexao.cursor()
    
    try:
        # Validação de Checagem: Mirar antes de atirar
        cursor.execute("SELECT nome FROM produtos WHERE id = ?", (id_produto,))
        ...
        # Execução da Exclusão com a trava do WHERE
...
def menu_interativo():
    ...
    while True:
        print("\n=== SISTEMA DE ESTOQUE ===")
        print("1. Adicionar novo produto (CREATE)")
        print("2. Listar produtos cadastrados (READ)")
        print("3. Atualizar preço de um produto (UPDATE)")
        ...
        if opcao == '1':
            ...
            while True:
                entrada_id = input("\nDigite o ID do produto a ser deletado (ou '0' para voltar): ").strip()
                ...
                try:
                    id_digitado = int(entrada_id)
                    # A função deletar_produto já tem as validações e mensagens embutidas nela
                    deletar_produto(id_digitado)
                except ValueError:
                    print("❌ Erro: Entrada inválida. Certifique-se de digitar apenas números!")
